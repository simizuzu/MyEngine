...
LevelData* LevelLoader::LoadFile(const std::string& fileName)
{
	...
	// "objects"の全オブジェクトを走査
	LoadObjectData(levelData,deserialized);

	// "objects"からfilenameで指定した敵の X,Y,Z を走査
	LoadEnemyData(levelData,deserialized);

	// "curves"の制御点の X,Y,Z を走査
	LoadCurvesData(levelData,deserialized);

	LoadAnimData(levelData,deserialized);

	return levelData;
}

void LevelLoader::LoadObjectData(LevelData* levelData,nlohmann::json deserialized)
{
	// "objects"の全オブジェクトを走査
...
void LevelLoader::LoadEnemyData(LevelData* levelData,nlohmann::json deserialized)
{
	// "objects"からfilenameで指定した敵の X,Y,Z を走査
	for ( nlohmann::json& enemys : deserialized[ "objects" ] )
	{
		...
		//生成時間を検索
		if ( enemys.contains("wait_time") )
		{
			enemyData.waitTime = enemys[ "wait_time" ];
		}
...
void LevelLoader::LoadCurvesData(LevelData* levelData,nlohmann::json deserialized)
{
	// "curves"の制御点の X,Y,Z を走査
...
void LevelLoader::LoadAnimData(LevelData* levelData,nlohmann::json deserialized)
{
